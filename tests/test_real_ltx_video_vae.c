...
int main(int argc, char **argv) {
    ...
    /* One ones vector, wide enough for every channel count either half uses. */
    float *ones_host = malloc(WIDEST_CHANNELS * sizeof(*ones_host));
    require(ones_host != NULL, "cannot allocate a norm weight");
    for (int index = 0; index < WIDEST_CHANNELS; index++) ones_host[index] = 1.0f;
    ...
    printf("LTX-2.5 video VAE, %u frames of %ux%u <-> [%d, %d, %d, %d]\n",
    ...
    require(latent_shape.depth == LATENT_FRAMES &&
            ...
            "the encoder did not reach the expected latent geometry");

    /* ---------------------------------------------------------- the decoder */

    /* Decoded from the *anchor's* latent, so a deviation here is the decoder's
     * own and not the encoder's carried forward. */
    shape decoded_shape;
    float *want_latent = golden("latent", volume(latent_shape));
    float *staged = malloc(volume(latent_shape) * sizeof(*staged));
    require(staged != NULL, "cannot allocate the staged latent");
    for (size_t position = 0; position < positions(latent_shape); position++)
        for (uint32_t channel = 0; channel < LATENT_CHANNELS; channel++)
    ...
    {
        float *want = golden("restored", volume(decoded_shape));
        double worst = 0.0, peak = 0.0;
        for (size_t index = 0; index < volume(decoded_shape); index++) {
            require(isfinite(frames[index]), "a pixel is not finite");
            const double delta = fabs((double)frames[index] - (double)want[index]);
            if (delta > worst) worst = delta;
            if (fabs((double)want[index]) > peak) peak = fabs((double)want[index]);
        }
        if (worst / peak > TOLERANCE) {
            fprintf(stderr, "FAIL %-28s %.3e = %.2e of peak\n", "frames",
                    worst, worst / peak);
            failures++;
        } else {
            printf("  ok  %-28s [%4u,%2u,%3u,%3u] %.3e = %.2e of peak %.4g\n",
                   "frames", decoded_shape.channels, decoded_shape.depth,
                   decoded_shape.height, decoded_shape.width, worst,
                   worst / peak, peak);
        }
        free(want);
    }
    ...
    /* Both halves composed, engine only, and scored the way a viewer would.
     * Every stage above is a comparison against a reference; this is the one
    ...
    square_error /= (double)volume(round_shape);
    const double psnr = 10.0 * log10(4.0 / square_error);
    printf("  ok  %-28s %.2f dB, engine encode and decode\n",
           "round trip", psnr);
    /* The reference manages 30.80 dB on this image. Anything near it means the
     * two halves invert each other; a wrongly wired pair lands at or below 0. */
    if (psnr < 25.0) {
        fprintf(stderr, "FAIL the engine's own round trip is %.2f dB, so the "
                "two halves do not invert each other\n", psnr);
        failures++;
    }
    ...
    printf("ok: both halves of the video VAE run on the released weights -- "
           "50 residual blocks, eight repackings, and a round trip that "
           "closes\n");
    return 0;
}
